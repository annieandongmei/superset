...
# A versioned import command that can be constructed from the file contents
# (and any extra dispatcher arguments) and run.
ImportCommandFactory = Callable[..., BaseCommand]


class BaseImportDispatcherCommand(BaseCommand):
    """
    Base class for import dispatcher commands.

    A dispatcher tries each command in ``command_versions`` until it finds one
    that can handle the given contents. Subclasses only need to define the
    ``command_versions`` list; ``v0`` (unversioned) commands, when supported,
    should be placed last since their files are not versioned.
    """
