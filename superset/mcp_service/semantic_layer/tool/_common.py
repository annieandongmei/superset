...
"""Shared helpers for the ``get_compatible_dimensions`` / ``get_compatible_metrics``
semantic layer tools.

Both tools follow the same shape: check metadata permission, require exactly one
of ``dataset_id`` / ``view_id``, then either load a built-in dataset (validating
the requested selection) or load an external semantic view (enforcing access).
Only the response-building differs, so that shared plumbing lives here.
"""
...
def check_metadata_permission() -> SemanticLayerError | None:
    """Return an error if the current user cannot view data model metadata."""
...
def validate_selection_target(
    dataset_id: int | None,
    view_id: int | None,
) -> SemanticLayerError | None:
    """Ensure exactly one of ``dataset_id`` / ``view_id`` was provided."""
...
def validate_builtin_selection(
    dataset: SqlaTable,
    selected_metrics: list[str],
    selected_dimensions: list[str],
) -> SemanticLayerError | None:
    """Validate that the requested metric/dimension names exist on the dataset."""
...
def load_external_view(
    view_id: int,
    action: str,
) -> SemanticView | SemanticLayerError:
    """Load an external semantic view and enforce access on it."""
