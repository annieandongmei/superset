...
def test_calculate_support_level_not_supported() -> None:
    """Test calculate_support_level returns 'Not supported' for no features."""
...
def test_calculate_support_level_time_grains() -> None:
    ...
    db_info = {"time_grains": {"SECOND": True, "MINUTE": False, "HOUR": True}}
    assert (
        calculate_support_level(db_info, ["time_grains.SECOND", "time_grains.MINUTE"])
        == "Partial"
    )
...
def test_infer_category_aws() -> None:
    ...
    assert infer_category("Amazon Redshift") == DatabaseCategory.CLOUD_AWS
    assert infer_category("aws Athena") == DatabaseCategory.CLOUD_AWS
...
def test_infer_category_gcp() -> None:
    ...
    assert infer_category("bigquery") == DatabaseCategory.CLOUD_GCP


def test_infer_category_azure() -> None:
    """Test infer_category detects Azure databases."""
    assert infer_category("Azure SQL") == DatabaseCategory.CLOUD_AZURE
    assert infer_category("microsoft sql server") == DatabaseCategory.CLOUD_AZURE
...
def test_infer_category_traditional_rdbms() -> None:
    ...
    assert infer_category("MySQL") == DatabaseCategory.TRADITIONAL_RDBMS
    assert infer_category("SQLite") == DatabaseCategory.TRADITIONAL_RDBMS


def test_infer_category_other() -> None:
    """Test infer_category returns OTHER for unknown databases."""
    assert infer_category("Unknown Database") == DatabaseCategory.OTHER
